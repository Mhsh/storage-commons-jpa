...
public interface JpaIngestionErrorDetailRepository extends JpaRepository<JpaIngestionErrorDetail, UUID> {
    ...
    @Query(value = "SELECT error_detail AS errorDetail, COUNT(*) AS count FROM error_ingestion_detail GROUP BY error_detail", nativeQuery = true)
